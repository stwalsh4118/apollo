...
| 2026-02-20 09:13:57 | Status Change | Proposed | Agreed | Task approved | AI_Agent |
| 2026-02-20 09:13:57 | Status Change | Agreed | InProgress | Started implementation | AI_Agent |
| 2026-02-20 09:21:54 | Status Change | InProgress | Review | Implementation complete, internal AI review passed | AI_Agent |
...
- 5 CoS tests pass covering all 9 acceptance criteria:
  - `TestCoS_AssemblerIngestionRoundTrip` — AC5, AC6, AC7: assembler → schema validation → ingest → DB queries
  - `TestCoS_FileSizeConstraint` — AC4: no fixture file exceeds 300 lines
  - `TestCoS_PartialProgress` — AC8: files persist after simulated partial pass 3 failure
  - `TestCoS_SystemPromptContent` — AC9: prompt contains file-writing instructions, directory conventions, no --json-schema
  - `TestCoS_Pass4Prompt` — pass 4 prompt references file tree review, not structured JSON
- AC1-AC3 structural: verified by existing `TestE2EHappyPath` (4 CLI calls, 1 initial + 3 resume)

### AI Review Notes
- Medium #1 (cosTestDB duplication): Accepted — package boundary between `research` and `research_test` prevents sharing the helper. Only 14 lines.
- Low #2, #4, #5, #6, #7: Fixed — improved comments, added scanner error check, removed redundant guard, checked MarshalIndent error, cleaned up placeholder table entry.

## Files Modified

- `api/internal/research/cos_test.go` (NEW) — 5 CoS tests covering AC4-AC9, with cosTestDB helper for internal-package DB access
