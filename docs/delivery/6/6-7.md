...
| 2026-02-19 08:57:10 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-19 09:10:00 | Status Change | InProgress | Review | Implementation complete, internal AI review passed | AI_Agent |
...
- 8 tests pass: happy path, retry success, unrecoverable failure, cancellation, schema validation failure, progress tracking, cancel method safety, no structured output
- Job lifecycle: queued → researching → resolving → published verified in happy path test
- Retry: pass 2 fails once, succeeds on retry, job completes
- Failure: pass 2 fails twice, job marked failed with error message
- Cancellation: job cancelled mid-pass, status set to cancelled gracefully
- Progress: ResearchProgress JSON updated with current pass (4/4) and pass descriptions
- `make check` passes (fmt, vet, all tests)

## Files Modified

- `api/internal/research/orchestrator.go` — Created: Orchestrator with Start(), RunJob(), Cancel(), 4-pass pipeline execution, retry logic, cancellation map, progress tracking
- `api/internal/research/orchestrator_test.go` — Created: 8 integration tests with mock CLIRunner
- `api/cmd/apollo/main.go` — Modified: Wire orchestrator with all dependencies, start background goroutine, connect cancel function to server
- `api/internal/repository/research.go` — Modified: Added FindOldestByStatus() to interface and implementation
- `api/internal/handler/research_test.go` — Modified: Added FindOldestByStatus() to mock repo
- `api/internal/config/config.go` — Modified: Added CurriculumSchemaPath config field and env var
- `api/internal/config/research.go` — Modified: Added DefaultCurriculumSchemaPath constant
