...
| 2026-02-19 08:38:11 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-19 08:41:20 | Status Change | InProgress | Review | Implementation complete, internal AI review passed | AI_Agent |
...
- JSON serialization round-trip tests pass for ResearchJob, ResearchProgress, CLIResponse
- Status constants match CHECK constraint values
- IsTerminalStatus correctly classifies all statuses
- `make check` passes (fmt, vet, tests)

## Files Modified

- `api/internal/models/research.go` — Created: ResearchJob, ResearchProgress, ResearchJobSummary, CreateResearchJobInput, ResearchJobStatus type, status constants, IsTerminalStatus
- `api/internal/models/cli.go` — Created: CLIResponse struct
- `api/internal/config/research.go` — Created: exported Default* constants, ResearchPassCount, ResearchSystemPromptFile, ResearchAllowedTools, ResearchPassDescription
- `api/internal/config/config.go` — Updated: unexported defaults now reference exported constants from research.go
- `api/internal/models/research_test.go` — Created: 6 unit tests covering JSON round-trip, omitempty, status constants, terminal status, CLI response parsing
