...
| 15 | Developer | As a developer, I want the research pipeline to produce a file-per-lesson directory tree so that each agent works with small, context-friendly files and Go code handles final assembly | InProgress | Pass 1 writes topic.json + module dirs; Pass 2 sub-agents write individual lesson files (~100-200 lines); Pass 3 adds exercises per-file; Go assembler merges tree into CurriculumOutput; schema validation and ingestion unchanged |
