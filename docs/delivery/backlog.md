...
| 5 | Developer | As a developer, I want a research skill prompt and curriculum JSON schema so that Claude Code sessions produce validated, structured curriculum output | Agreed | Research skill prompt implements 4-pass pipeline; JSON schema matches PRD section 7; validation logic rejects malformed output; schema file usable with --json-schema flag |
