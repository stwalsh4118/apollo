...
| 8 | enhancement | Auto-generate prompt type definitions from JSON schema | PBI-16 | 2026-02-20 | Task 16-1 manually transcribes schema types into the prompt. A build step or code generator could read `schemas/curriculum.json` and emit the type definitions section automatically, keeping prompt and schema in sync. |
| 9 | enhancement | Sanitizer metrics/counters for observability | PBI-16 | 2026-02-20 | SanitizeDir logs per-field warnings but doesn't aggregate counts. A summary struct (files processed, fields stripped, by type) would be useful for monitoring prompt drift over time across many pipeline runs. |
| 10 | tech-debt | ContentSection type dispatch uses string matching | PBI-16 | 2026-02-20 | The sanitizer must dispatch on `type` field to choose the right allowed-fields set for ContentSection variants. This duplicates the oneOf logic in the schema. Consider generating dispatch tables from the schema. |
